...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function sendMessageToBackground(action, data = {}) {
    chrome.runtime.sendMessage({ action: action, ...data }, (response) => {
      if (chrome.runtime.lastError) {
        ...
        statusElement.textContent = "Error: Could not communicate with background script.";
        return;
      }
      if (response && response.status) {
        // Update status from background script (e.g., "started", "stopped", "progress")
        statusElement.textContent = `Status: ${response.message || response.status.charAt(0).toUpperCase() + response.status.slice(1)}.`;
      }
    });
  }
  ...
  chrome.runtime.sendMessage({ action: "getTabs" }, (response) => {
    if (chrome.runtime.lastError) {
      console.error("Error getting tabs:", chrome.runtime.lastError.message);
      tabListContainer.innerHTML = '<p class="text-center text-red-500">Error loading tabs.</p>';
      return;
    }
    if (response && response.tabs) {
      availableTabs = response.tabs;
      windowsData = response.windows || [];
  ...
  function renderTabsByWindow(windows) {
    ...
    if (windows.length === 0) {
      tabListContainer.innerHTML = '<p class="text-center text-gray-500">No scriptable tabs found.</p>';
      return;
    }
    ...
    windows.forEach((window, windowIndex) => {
      ...
      window.tabs.forEach(tab => {
        ...
        label.textContent = tab.title ? tab.title.substring(0, 60) + (tab.title.length > 60 ? '...' : '') : tab.url;
      ...
      selectWindowCheckbox.addEventListener('change', (event) => {
        const windowCheckboxes = windowTabsContainer.querySelectorAll('.tab-checkbox');
  ...
  function updateSelectAllState() {
    ...
    document.querySelectorAll('.window-checkbox').forEach(windowCheckbox => {
      ...
      const windowTabs = document.querySelectorAll(`[data-window-id="${windowId}"].tab-checkbox`);
      const checkedWindowTabs = document.querySelectorAll(`[data-window-id="${windowId}"].tab-checkbox:checked`);
  ...
  startButton.addEventListener('click', () => {
    const selectedTabIds = Array.from(document.querySelectorAll('.tab-checkbox:checked'))
    ...
    // Get print settings
    const printInterval = Math.max(2, Math.min(60, parseInt(printIntervalInput.value) || 3));
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.status) {
      statusElement.textContent = `Status: ${request.message || request.status.charAt(0).toUpperCase() + request.status.slice(1)}.`;
    }
  });
